...
Dado que ahora el $\rotational{\E} \neq \textbf{0}$, es necesario estudiar las componentes tangenciales de $\E$ en una superficie de discontinuidad, para ello consideramos un pequeño elemento de superficie en la capa de transición que sea perpendicular al flujo, tal como se ilustra en la figura (\ref{}), donde $\hat{\textbf{n}}\textbf{'}$ es el vector unitario normal a la superficie encerrada por la trayectoria $C$ y paralelo a la superficie entre los medios $1$ y $2$, $\hat{\textbf{n}}$ es el vector unitario perpendicular a la  superficie, y el vector unitario $\hat{\textbf{t}}$ es paralelo al plano de $C$, de esta forma la integral de trayectoria $\E$ alrededor de $C$ esta dada por:
...
\begin{equation}
	...
	\oint_{C} \E \cdot \dl \textbf{l} = - h \; \Delta L \diffp{\B}{t} \cdot \hat{\textbf{n}} \textbf{'}.
\end{equation}

Igualando \eqref{Eq:ComponenteTangencialE} y \eqref{Eq:ComponenteTangencialB} obtenemos

\begin{equation}
	\hat{\textbf{t}} \cdot \left( \E_{2} - \E_{1} \right) \, \Delta L + \kappa + \diffp{\B}{t} \cdot \hat{\textbf{n}}' \, h\, \Delta L = 0.
\end{equation}
...
\begin{equation}
	\left( \hat{\textbf{n}} \times \hat{\textbf{n}}\textbf{'} \right) \cdot \left( \E_{2} - \E_{1} \right) \, \Delta L + \diffp{\B}{t} \cdot \hat{\textbf{n}}\textbf{'} \ ,h \, \Delta L = -\kappa.
\end{equation}
...
\begin{align*}
	-\kappa &= \left[ \left( \E_{2} - \E_{1} \right) \times \hat{\textbf{n}} \right] \cdot \hat{\textbf{n}}\textbf{'} \, \Delta L + \diffp{\B}{t} \cdot \hat{\textbf{n}}\textbf{'} \, h \,\Delta L,\\
					&= \left[ \left( \E_{2} - \E_{1} \right) \times \hat{\textbf{n}} + \diffp{\B}{t} \, h \right] \cdot \hat{\textbf{n}}\textbf{'} \Delta L.
\end{align*}

Si ahora hacemos que $h\rightarrow 0$ entonces $\kappa\rightarrow 0$, de esta forma

\begin{equation}
	\left[ \left( \E_{2} - \E_{1} \right) \times \hat{\textbf{n}} + \lim_{h\rightarrow 0} \left( \diffp{\B}{t} \, h \right) \right] \cdot \hat{\textbf{n}}\textbf{'} \Delta L = 0,
\end{equation}
